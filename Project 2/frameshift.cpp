...
std::string DNA_strand_frameshift(std::string DNA_strand){
  std::string result="";
  for (int i = 0; i < DNA_strand.length()-2; i++){
    if(DNA_strand.substr(i,3)=="TAC"){
      result = DNA_strand.substr(i,DNA_strand.length()-i);
      break;
    }
  }
...
int main(){
  std::fstream file ("frameshift_mutations.txt");//First file used to read the DNA strands
  ...
  while(getline(file,  DNA_strand)){ //Will run until all DNA strands are read
    std::cout<<DNA_strand_to_Polypetide_chain(DNA_strand_frameshift(DNA_strand))<<std::endl;
  }
  return 0;
}
